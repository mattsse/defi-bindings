...
mod iflashloanreceiver_mod {
    ...
    use std::sync::Arc;
    pub static IFLASHLOANRECEIVER_ABI: ethers::contract::Lazy<ethers::core::abi::Abi> =
    ...
    pub static IFLASHLOANRECEIVER_BYTECODE: ethers::contract::Lazy<ethers::core::types::Bytes> =
    ...
    impl<M: ethers::providers::Middleware> std::fmt::Debug for IFlashLoanReceiver<M> {
    ...
    impl<'a, M: ethers::providers::Middleware> IFlashLoanReceiver<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), IFLASHLOANRECEIVER_ABI.clone(), client)
                .into()
        }
        ...
        > {
            let factory = ethers::contract::ContractFactory::new(
                ...
                IFLASHLOANRECEIVER_BYTECODE.clone().into(),
                client,
            );
    ...
    impl ethers::core::abi::AbiDecode for IFlashLoanReceiverCalls {
        fn decode(data: impl AsRef<[u8]>) -> Result<Self, ethers::core::abi::AbiError> {
            ...
            {
                return Ok(IFlashLoanReceiverCalls::AddressesProvider(decoded));
            }
            if let Ok(decoded) = <PoolCall as ethers::core::abi::AbiDecode>::decode(data.as_ref()) {
                return Ok(IFlashLoanReceiverCalls::Pool(decoded));
            }
            if let Ok(decoded) =
                <ExecuteOperationCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(IFlashLoanReceiverCalls::ExecuteOperation(decoded));
            }
    ...
    impl ethers::core::abi::AbiEncode for IFlashLoanReceiverCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                IFlashLoanReceiverCalls::AddressesProvider(element) => element.encode(),
                IFlashLoanReceiverCalls::Pool(element) => element.encode(),
                IFlashLoanReceiverCalls::ExecuteOperation(element) => element.encode(),
            }
        }
    }
    impl ::std::fmt::Display for IFlashLoanReceiverCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                IFlashLoanReceiverCalls::AddressesProvider(element) => element.fmt(f),
                IFlashLoanReceiverCalls::Pool(element) => element.fmt(f),
                IFlashLoanReceiverCalls::ExecuteOperation(element) => element.fmt(f),
            }
        }
    }
    impl ::std::convert::From<AddressesProviderCall> for IFlashLoanReceiverCalls {
        fn from(var: AddressesProviderCall) -> Self {
            IFlashLoanReceiverCalls::AddressesProvider(var)
        }
    }
    impl ::std::convert::From<PoolCall> for IFlashLoanReceiverCalls {
        fn from(var: PoolCall) -> Self {
            IFlashLoanReceiverCalls::Pool(var)
        }
    }
    impl ::std::convert::From<ExecuteOperationCall> for IFlashLoanReceiverCalls {
        fn from(var: ExecuteOperationCall) -> Self {
            IFlashLoanReceiverCalls::ExecuteOperation(var)
        }
    }
}
